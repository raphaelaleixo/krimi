      ...
      <v-col cols="12">
        <v-card v-if="game.murdererChoice">
          <v-card-title>
            ...
            >&nbsp;&nbsp;{{game.murdererChoice.mean}}</span> to kill, and left behind
            <span
              class="red--text mx-2"
            >&nbsp;&nbsp;{{game.murdererChoice.key}}</span> as a key evidence.
          </v-card-title>
        </v-card>
        <v-card v-else>
          <v-card-title>{{murderer.name}} is the murderer. Wait for means of murder and key evidence.</v-card-title>
        </v-card>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12" md="6" lg="3" v-for="(item, index) in availableClues" :key="index">
        <v-card>
          <v-card-text>
            {{ item.title }}
            <v-select
      ...
      <v-col class="mt-6" cols="12" v-if="game.murdererChoice">
        <v-btn
          @click="sendAnalysis"
          :disabled="analysis.length < availableClues.length || game.forensicAnalysis && game.forensicAnalysis.length === availableClues.length"
        >Send analysis</v-btn>
      </v-col>
    </v-row>
  </div>
